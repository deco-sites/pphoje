    ...
    {prog?.map((pr, index)=>(
        pr.days.map((x, index) =>(
            x.attraction.map((y, index)=>(
                <div class="mb-[1px]">
                    <div class="mx-auto py-2 mt-4 flex w-3/4 gap-2 rounded-md" style={{ "box-shadow": "0 1px 1px rgba(0, 0, 0, 0.5), 0 2px 8px rgba(0, 0, 0, 0.5), inset 0px 0px 0px 1px rgba(255, 255, 255, 0.1)"}}>
                        <div class="pl-2 pt-2">
                            <img src={y.mobile} alt="" width={60}
                                height={60}  class="rounded-full w-[62px] h-[62px]" />
                        </div>
                        <div class="flex flex-col pl-2">
                            <span class="text-card-date">{y.date}12</span>
                            <span class="text-card-name">{y.label}</span>
